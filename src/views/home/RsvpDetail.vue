<template>
  ...
  <ion-page>
    <ion-content class="ion-padding">
      <ion-card style="margin-top: 80px">
        <ion-card-header>
          <div class="icon-container">
            <ion-icon
              ...
              class="success-icon"
              v-if="rsvp.payment_status == 'paid'"
            ></ion-icon>
            <ion-icon
              :icon="warningOutline"
              class="warning-icon"
              v-if="rsvp.payment_status == 'unpaid'"
            ></ion-icon>
            <ion-icon
              :icon="closeCircleOutline"
              class="danger-icon"
              v-if="
                rsvp.payment_status !== 'unpaid' &&
        ...
        <ion-card-content>
          ...
          <div class="flex justify-between">
            ...
            <span>
              <b>{{ new Date(rsvp.created_at).toLocaleString() }}</b></span
            >
          </div>
          ...
          <div class="flex justify-between" style="margin-top: 3px">
            ...
            <span
              ><b>{{ currencyIDR(rsvp.total) }}</b></span
            >
          </div>
          <div class="flex justify-between" style="margin-top: 3px">
            <span>
      ...
      <ion-card>
        <ion-card-header>
          <ion-card-subtitle> Reservation Table(s) Detail </ion-card-subtitle>
        </ion-card-header>
        <ion-card-content>
          <ion-list>
            <ion-item v-for="(tbl, idx) in tablesOutlet" :key="idx">
              <ion-label>
                {{ getOrdinalSuffix(tbl.floor) }} Floor - Table No :
                {{ tbl.table }} - Max Pax : {{ tbl.max_pax }} - @{{
                  currencyIDR(tbl.price)
                }}
              </ion-label>
            </ion-item>
          </ion-list>
...
const getDetail = async () => {
  ...
  const id: number = parseInt(String(prop.id));
  const resp: any = await getRsvpDetail(id);
